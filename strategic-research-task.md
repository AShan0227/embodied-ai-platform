...
### 6.7 常见战略误判：哪些动作最容易把方向做偏

| 常见误判 | 为什么危险 | 更合理的替代动作 |
|----------|------------|------------------|
| 一开始就做“通用 Agent 平台” | 同时与模型平台、云厂商和应用层竞争，资源需求过大 | 先选 `运行时/控制面` 或一个高价值垂直工作流切入 |
| 把模型能力领先当成主要壁垒 | 模型层迭代快，领先窗口短，上层价值容易被替换 | 把壁垒放在治理、连接器、流程资产和企业嵌入度 |
| 先做 demo，治理以后再补 | 一旦进入企业流程，权限、审计、回放缺失会直接阻塞采购 | 从第一版就把 tracing、权限边界、审批点设计进去 |
| 同时铺太多场景 | 会导致连接器、评测、流程理解都不够深，难以形成 ROI 闭环 | 先把一个窄场景做深，形成可复制模板后再扩张 |
| 把多 Agent 当作默认答案 | 角色边界不清时，多 Agent 只会增加成本和调试复杂度 | 只有在职责明确、接口清晰时才引入多角色系统 |

这部分的核心不是“避免保守”，而是避免把正确的行业判断执行成高复杂度、低胜率的组织动作。
...
- 2026-03-15（UTC）对附录 23 个来源做了一轮仓库内自动校验。
- 来源 `7-23` 返回 `200`，可正常解析到目标页面。
- 来源 `1-6`（OpenAI 域名）对 `curl` 请求返回 `403`；这更像站点的自动化访问限制，而不是文档链接失效，因此仍保留为官方规范 URL。
- 这意味着本文的主要风险不在“引用大面积失效”，而在少数站点对机器人校验不友好；人工浏览器复核时应优先关注 OpenAI 相关链接。
